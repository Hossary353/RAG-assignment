...
public class ChatSessionController {
    ...
    public ResponseEntity<SessionResponse> markFavourite(@RequestBody @Validated SessionRequest sessionRequest, @PathVariable boolean favourite){
        return new ResponseEntity<>(chatSessionService.markFavourite(sessionRequest,favourite), HttpStatus.OK);
    }

    @PutMapping("/rename/{name}")
    public ResponseEntity<SessionResponse> rename(@RequestBody @Validated SessionRequest sessionRequest, @PathVariable String name){
